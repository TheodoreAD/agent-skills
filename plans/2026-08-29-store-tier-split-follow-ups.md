...
It left four things undone, and this file is what carries them — `plans.py list` surfaces it,
because `in-progress` sorts above everything else.
...
Nothing here is durable reasoning. When the four are done this file is retired outright, with no
`## Migrated to` section to write — that is the difference between a working list and a plan, and
the reason it is one file rather than four.
...
### 3. Absorb the ssh plan filed for `power-user-linux-setup`

Only a session inside that repo can do it. It answers an open `[NEEDS CLARIFICATION:]` in a plan
already there, so `absorb` should pair them.

- Check: `plans.py absorb`, from inside that repo
- Act: absorb, consolidate with `2026-08-29-ssh-prefix-applied-without-a-failure.md`, then make the
  `~/AGENTS.md` fragment edit it describes and deploy it.
...
carries a recorded reason not to be, and `absorb` is silent in `power-user-linux-setup`. Then delete
this file.
