...
allowlist (the `[inv]` entry's `allow_overrides` in `power-user-linux-setup`'s
`cli-allowlist/tools.toml`, rendered per harness by `inv allowlist.apply`) auto-approves
`inv <ns>.status|check|diff|list|show`, `<ns>.print-*`, `<ns>.check-*`, `<ns>.list-*` and
`<ns>.*-check` in every repo, on the strength of the convention alone. A task with one of these
names must inspect and never mutate — a `deploy.check` that also fixes what it finds would run
unprompted. Give a mutating task a mutating name (`fix`, `apply`, `sync`, `configure`).
