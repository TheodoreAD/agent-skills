...
description: "Use when developing or distributing a personal MCP server or Agent Skill repo (e.g. the `*-polite-mcp` family, `product-research-pipeline`), or deciding how to register one with an agent — dev-loop setup, git-as-artifact-store distribution via `uv tool install` instead of PyPI, local-vs-deployed MCP registration and why one registered command covers both, and how a skill repo ships through the `skills` CLI rather than a bespoke installer."
---
...
## Skills ship through the `skills` CLI — don't build a new mechanism

A skill repo (like `product-research-pipeline`'s orchestrator skill) needs none of the above. Put
the skill at `skills/<name>/SKILL.md` in a public repo — no manifest, no marketplace entry, no
vendor directory — and it is installable by anyone:

```shell
npx skills add TheodoreAD/<repo> --global            # every skill in the repo
npx skills add TheodoreAD/<repo> --skill <name>      # just one
```

The [`skills` CLI](https://github.com/vercel-labs/skills) discovers skills up to three levels deep
in the standard locations, copies the chosen ones into the canonical `~/.agents/skills/` hub, and
links each detected agent at it. A skill that is a repo's _own_ interface documentation belongs
committed in that repo under `.agents/skills/`; a cross-project convention skill belongs in a
dedicated skills repo. Both are installed by the same command.

[PITFALL: **The CLI announces a Claude Code symlink it does not always create.** Measured
2026-08-27: installing alongside any universal agent prints `symlink → Claude Code` in the summary
and leaves no `.claude/skills` behind. Claude Code reads `.claude/skills`, not `.agents/skills`, so
verify that link exists rather than trusting the install summary — `ln -s` it at the hub yourself if
it doesn't.]

Two properties worth knowing before building tooling around this: installs are **copies**, never
links to the source checkout, so an edit needs a re-install to take effect; and the source is cloned
from the remote, so an unpushed edit is invisible. Iterating on a skill means either pushing each
time or pointing the CLI at a local path (`skills add ../my-skills`), which it accepts as a
first-class source.
...
decision it made, ask rather than guessing — then fold the resolution back into the skill's own
source as a small, additive edit, and re-install so the fix reaches every project. Never edit the
installed `~/.agents/skills/<name>` copy: it is a plain copy, so the edit is clobbered by the next
install and reaches no other machine in the meantime. The `session-harvest` skill is the worked
example — its "On friction, ask" and "Self-update mechanics" sections are the pattern to copy when
...
`uvx`/`uv
run` draft that came first), why `--scope user`, why one registered command across two
install sources, and why "self-update on friction" generalizes rather than being one skill's quirk.
