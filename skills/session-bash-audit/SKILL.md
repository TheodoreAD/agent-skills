...
| A command shape prompts but is read-only and common                                                | The permission allowlist, as a prefix rule (a generated `tools.toml` entry). Not for `git -C`-style shapes on Claude Code: no prefix rule can match them there, so that is the sandbox's job |
