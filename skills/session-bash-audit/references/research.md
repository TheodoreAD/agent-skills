...
**These are one machine's measurements**, taken on the author's setup with its own permission
allowlist, instruction file and repos, and they name that machine's config files by path
(`tools.toml`, `setup.toml`, `contributing/*.md` — all in
[`power-user-linux-setup`](https://github.com/TheodoreAD/power-user-linux-setup)). Read them as a
worked example of the method and as a reference point to compare a run against, not as your own
baseline. The findings that generalize — root causes, harness facts, why a nudge hook was rejected —
are the reason this file ships with the skill; the specific rule edits are not portable and are not
meant to be.
...
## `--save-baseline` destroyed the baseline it was meant to be compared against (2026-09-05)

The default filename was a **UTC** date. This machine runs at `+03:00`, so a run at 02:13 local on
2026-09-05 wrote `2026-09-04.json` — the name the previous afternoon's baseline already had. It was
overwritten with no prompt, no backup and no mention that anything was there, the only output being
`baseline written to …` on the last line of a several-hundred-line report, which reads as success.
The file destroyed was the anchor an `[UNVERIFIED:]` in another repo explicitly named, for measuring
whether a wording change had moved the `head`/`tail` rate. It survived only by luck: that question
had already been answered in prose before the file was lost.

Two decisions came out of it, and the second is the one that reads as odd without the first:

- **Refuse, rather than auto-suffix.** It matches how this corpus treats every other one-way write,
  and a refusal is readable in a way an accumulating `-2` is not: **the whole point of the default
  path is that nobody names it, so nobody would notice a second file appearing under a name they
  never chose either.** `--force` is the deliberate destroy, and the refusal names the existing
  file's `saved`, `days`, `note` and `instrument` so the caller sees what they were about to lose.
- **The UTC filename stays; the local timestamp goes in `saved`.** Local-dating the file was the
  obvious fix and is the wrong one — an artefact already on disk keeps its scheme, and once the
  collision is a refusal rather than a loss, the naming was never the half that cost anything.

The reason this is worth a section rather than a changelog line: **the thing overwritten is by
construction irreplaceable.** A baseline measures a corpus that has since moved on, so unlike almost
every other file a tool writes, it cannot be regenerated by running the tool again. Reconstruction
was attempted here and is labelled a reconstruction in its own `note`, because `--days` counts back
from _now_ rather than from `--until`, so the window approximates the original rather than
reproducing it.

The same write now records `instrument` — the script's own short SHA, with `-dirty` when its
checkout has uncommitted changes to it, and `None` when it did not run from a checkout, which is the
honest answer rather than a SHA borrowed from whatever repo the installed copy happens to sit under.
That is three read-only `git` calls in a script whose disclosure had said it ran nothing, so the
disclosure says so and scopes them to the write path.

**`--note` was left optional**, deliberately and against the instinct to require it: `saved` now
carries a local timestamp with its offset and `instrument` names the version that wrote the file, so
a note-less baseline is identifiable even if it is not self-explaining — and requiring one would
make the ordinary case, a quick baseline before a change, prompt for prose nobody has yet.

## Why there is a Windows CI leg for a platform nobody here runs (2026-09-05)

Reversing a decision taken the day before. The argument against had been that a runner could measure
only three things and would settle neither of the two questions that need a real Claude Code session
on Windows, so "declared, not measured" was the honest position. The reversal: without any run on
Windows every claim in the corpus stays declared, and three measured things is three more than zero.

**The prediction was wrong in an instructive way, and that is the argument for the leg in one
sentence.** The expected first failure was a path-string assertion. The actual one was TOML escaping
— a fake config wrote a Windows path inside a TOML basic string, where a backslash opens an escape,
so `C:\Users` is an invalid `\U` — and it took a hundred plan-store tests down through one helper
line. 136 red on the first run from four causes, none of them predictable by reading the tests on
the other platform: that escaping, files read through the platform code page turning an em dash into
mojibake and pushing a description four characters over its cap, fixtures keying commands on POSIX
separators while `Path` renders backslashes, and two suites reading the real platform constant where
they meant the POSIX arm. Plus a workflow setting `autocrlf` _after_ the checkout, which is the only
order in which it does nothing.

Two findings were in the scripts rather than the tests, which is the part a reviewer would have been
least likely to reach: three reads relied on the platform encoding, so a transcript containing an em
dash would have raised on Windows; and `doctor` joined a store path and a root name with a literal
slash. A third bug the leg exposed was not Windows-specific at all — the project-slug function
replaced only `/` and `.` where the harness slugs `[^a-zA-Z0-9]`, so a repo path containing `_`
never matched and the own-repo rows had been reporting zero **on Linux too**.

**So the failures a platform produces are not the ones a reviewer on the other platform imagines**,
and that is what a leg buys even when it cannot answer the interesting questions. The two it still
cannot answer are recorded as open rather than resolved: whether Claude Code's Bash tool on Windows
